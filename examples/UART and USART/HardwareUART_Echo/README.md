...
ただし __MicroAPI__ で提供される組込UART制御は（割込を使用しない）「半二重通信」であることに注意を要する。
...
> [!TIP]
> 割込不使用の半二重通信は、割込ベクター内からでも短文送信が可能であるという利点がある。
> これはデバッグ手段が限られた開発環境では大いに役立つ。

> [!NOTE]
> 実際のこのスケッチには初期動作確認用として LED点滅や MPUリセット機能が追記されている。
