...
65536000L未満かつ
1000で割り切れなければならない。
従って```OSC32K```を主クロック元に選ぶと
正しい時間精度は得られない。

```OSC32K```等の超低速駆動用 delay は
[Blink_08_delay_SLOW.ino](../Blink_08_delay_SLOW/README.md) を参照のこと。
...
ところで```F_CPU```が特定の値（```pow(2, n) * 1000000L```に一致）
である場合はこの演算をもっと単純化できる。
...
> シフト演算に置き換えても avr-gcc組込乗除算より高度な最適化ができる状況は稀だろう
...
> ```yield()```はweek修飾宣言された空関数なので、任意で再宣言できる。\
>\
> この処理中に```micros()```が巻き戻って```start_us```より小さくなっても
内側のループは抜けることができるので暴走はしない。
