...
namespace Timer {
  inline void init (void) {
    TCB4_CCMP = F_CPU / 1000 - 1;
    TCB4_INTCTRL = TCB_CAPT_bm;
    TCB4_CTRLB = TCB_CNTMODE_INT_gc;
    TCB4_CTRLA = TCB_ENABLE_bm | TCB_CLKSEL_DIV1_gc;
  }
  ...
  uint32_t micros (void) {
    ...
    ATOMIC_BLOCK(ATOMIC_RESTORESTATE) {
      _ms = _timer_millis;
      _tc = TCB4_CNT;
      if (bit_is_set(TCB4_INTFLAGS, TCB_CAPT_bp)) {
        _tc = TCB4_CNT;
        _ms++;
      }
    }
